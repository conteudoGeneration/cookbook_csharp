...
4. O conteúdo do arquivo **appsettings.json** ficará se semelhante a imagem abaixo: 
...
| Atributo                     | Descrição                                                    |
| ---------------------------- | ------------------------------------------------------------ |
| **Server**                   | Indica o endereço do servidor, Como estamos trabalhando localmente, podemos utilizar **localhost** (equivalente a máquina locala - 127.0.0.1) ou podemos utilizar o nome da máquina (hostname). Para consultar o nome da sua máquina, abra o Terminal e digite o comando `hostname`. |
...
A maneira mais recomendada para trabalhar com o contexto é definir uma classe que herda a Classe  **DbContext** e expõe as propriedades de **DbSet** que representam as coleções das entidades especificadas no contexto. No próximo conteúdo, vamos criar a nossa primeira Classe Model e trabalharemos com a Classe **DbSet**.
