...
5. Registrar a Classe Validator TemaValidator na Classe Program
6. Registrar a Classe Model Tema na Classe AppDbContext
...
| Método              | Descrição                                                    |
| ------------------- | ------------------------------------------------------------ |
| **HasMany()**       | Configura o lado *Muitos* de uma relação muitos-para-um ou o primeiro lado muitos de uma relação muitos-para-muitos. |
| **HasOne()**        | Configura o lado *Um* de uma relação um-para-muitos ou o primeiro lado um de uma relação um-para-um. |
| **WithMany()**      | Configura o lado *Muitos* de uma relação um-para-muitos ou o segundo lado muitos de uma muitos-para-muitos. O método **WithMany()** deve ser usado em conjunto com o Método **HasOne()** ou **HasMany()**. |
| **WithOne()**       | Configura o lado *Um* de uma relação um-para-um ou muitos-para-um. O método **WithOne()** deve ser usado em conjunto com o Método **HasOne()** ou **HasMany()**. |
...
| Muitos pra Um - N:1     | 🡪    | Muitos Objetos da Classe B se relacionarão com um Objeto da Classe B. Na prática é a mesma coisa do Relacionamento Um pra muitos - 1:N, apenas a ordem de construção que foi invertida. |
| Muitos pra Muitos - N:N | 🡪    | Muitos Objetos da Classe A se relacionarão com muitos Objetos da Classe B. |
...
5. A imagem abaixo, mostra como ficará o trecho com a nova linha:
 ...
 <div align="left"><img src="https://i.imgur.com/XF4hBjF.png" title="source: imgur.com" /></div>

Vamos entender o código:

**Linhas 25 a 32:** Registra o Pacote **Newtonsoft.Json** como um serviço na Classe **Program**.

**Linha 29:** Esta linha define a Propriedade **ReferenceLoopHandling** como **Ignore**, para que os valores de loop sejam ignorados da serialização ao invés de gerar uma exceção.

**Linha 30:** Esta linha define a Propriedade **NullValueHandling** como **Ignore**, para que todos os atributos com valores nulos sejam ignorados da serialização.
...
<div align="left"><img src="https://i.imgur.com/GRl4C55.png" title="source: imgur.com" /></div>

**Linha 18:** A Anotação **[InverseProperty("Tema")]** foi utilizada porque a Classe Postagem terá 2 Relacionamentos (Tema e futuramente User). Para identificar qual relacionamento o Atributo **Postagem** está associado, utilizamos a propriedade **InverseProperty** para indicar a **Entidade Inversa da Relação**.
...
5. A imagem abaixo, mostra como ficará o Método com as novas linhas:
 ...
 <div align="left"><img src="https://i.imgur.com/HNgANN3.png" title="source: imgur.com" /></div>

Vamos entender o código:

**Linhas 16 a 21:** Definimos o Relacionamento Bidirecional entre as Classes Tema e Postagem.

**Linha 16:** Escolhemos configurar o Relacionamento a partir da Classe Entidade Filha. Por isso selecionamos a Model Postagem na instrução **modelBuilder.Entity<Postagem>()**. 

**Linha 17:** Através do Método **HasOne(p => p.Tema)** especificamos que a Entidade Postagem terá um Atributo chamado **Tema**, Objeto da Classe Tema. Esse Objeto representa a Chave Estrangeira. Na sequência, precisamos configurar a outra ponta do relacionamento, a Classe Model Tema. 

**Linha 18:** Através do Método **WithMany(t => t.Postagem)** especificamos que a classe Tema terá um Atributo, que será uma Collection, que armazenará diversos Objetos da Classe Postagem, associados ao Objeto da Classe Tema. Na prática, o Método **WithMany()** indica qual Atributo será a **Propriedade de Navegação da Classe Tema**. 

**Linha 19:** Através do Método **HasForeignKey("TemaId")** especificamos o nome do atributo, que será a Chave Estrangeira na tabela **tb_postagens**. 

**Linha 20:** Os relacionamentos entre Entidades geralmente dependem da existência de outra Entidade, por exemplo, o relacionamento **Tema 🡪 Postagem**, sem um Objeto Tema associado, a Entidade Postagem não tem nenhum significado próprio, se tornando o que chamamos de **Objeto Órfão** (Orphan Object). **Quando excluímos um Objeto da Entidade Tema, todos os Objetos da Entidade Postagem associadas ao Tema excluído também devem ser excluídos**, entretanto, o inverso não é verdadeiro. **"Cascatear" (cascade)**, é a maneira de conseguirmos este efeito no Relacionamento. Quando  executamos alguma ação na Entidade Tema, a mesma ação será aplicada à  Entidade associada em Postagem.
...
<div align="left"><img src="https://i.imgur.com/JACNZiR.png" title="source: imgur.com" width="5%"/> <a href="https://github.com/rafaelq80/backend_blogpessoal_aspnet_v7/tree/11_tema_relacionamento_postagem" target="_blank"><b>Código fonte do projeto</b></a></div>
